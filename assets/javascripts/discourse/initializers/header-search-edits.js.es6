                    ...
                    if (searchMenu.length > 0) {
                        ...
                        if (customBlock.length == 0 && results.length > 0 && noResults.length == 0 ) {
                            $($('.panel-body')[0]).append('<div class="custom-block">' +
                                ...
                                '<button id="create-topic" onclick="alert("ok");" class="btn btn-default btn btn-icon-text ember-view">  <svg class="fa d-icon d-icon-plus svg-icon svg-string" xmlns="http://www.w3.org/2000/svg"><use xlink:href="#plus"></use></svg>' +
